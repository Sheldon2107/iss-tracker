...
def fetch_and_store_iss_data():
    """Background task to fetch ISS data periodically"""
    while True:
        try:
            ...
            # Respect API rate limit (1 req/sec) - fetch every 60 seconds
            time.sleep(60)
...
def home():
    return jsonify({
        "message": "ISS Tracker API",
        "endpoints": {
            "/current": "Get current ISS position",
            "/history": "Get historical data",
            "/analytics": "Get analytics (min/max values)",
            "/health": "Health check"
        }
    })
...
@app.route('/history')
def get_history():
    """Get historical ISS data with optional limit"""
    try:
        ...
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        c.execute(f'''SELECT * FROM iss_telemetry 
...
@app.route('/analytics')
def get_analytics():
    """Get analytics: min/max longitude, altitude changes"""
    try:
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        
        # Get min/max longitude
        ...
        # Get first and last altitude for change calculation
...
if __name__ == '__main__':
    ...
    data_thread = threading.Thread(target=fetch_and_store_iss_data, daemon=True)
    data_thread.start()
